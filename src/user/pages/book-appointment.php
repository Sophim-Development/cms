...
if (!isset($_SESSION['user_id'])) {
  ...
  if (isset($_SESSION['user_id'])) {
    ...
    dirname(__DIR__, 2) . '/includes/header-public.php';
  }
}
?>
...
<div class="container mx-auto mt-12 max-w-md">
  ...
  <?php if ($message): ?>
    <p class="mb-6 text-center <?php echo strpos($message, 'success') !== false ? 'text-green-500' : 'text-red-500'; ?>">
      <?php echo $message; ?></p>
  <?php endif; ?>
  ...
  <?php if (!isset($_SESSION['user_id'])): ?>
    <div class="bg-white p-6 rounded-lg shadow-lg text-center">
      ...
      <a href="/user/pages/login.php"
        class="bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700 transition">Login Now</a>
    </div>
  <?php else: ?>
    <div class="bg-white p-6 rounded-lg shadow-lg">
      <form method="POST">
        ...
        <button type="submit" class="w-full bg-blue-600 text-white p-3 rounded-lg hover:bg-blue-700 transition">Book
          Appointment</button>
      </form>
    </div>
  <?php endif; ?>
...
<?php
if (isset($_SESSION['user_id'])) {
  dirname(__DIR__, 2) . '/includes/footer-auth.php';
} else {
  dirname(__DIR__, 2) . '/includes/footer-public.php';
}
?>